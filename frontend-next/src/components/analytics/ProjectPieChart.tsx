...
const CustomTooltip = ({ active, payload }: any) => {
  ...
  return (
    <div
      className="rounded-xl border border-line px-3.5 py-2.5 text-xs shadow-lg backdrop-blur-xl"
      
    >
      <div className="font-medium text-fg text-sm mb-1">{d.projectName}</div>
      <div className="flex items-center gap-2">
        <div className="w-2 h-2 rounded-full" style={{ backgroundColor: d.color }} />
        <span className="text-fg-muted">{d.total} تسک</span>
      </div>
    </div>
  );
};
...
export default function ProjectPieChart({ data, height = 300 }: ProjectPieChartProps) {
  ...
  const dataWithColors = data.map((d, i) => ({
    ...d,
    color: d.color || c.series[i % c.series.length],
  }));
      ...
      <div className="flex flex-wrap justify-center gap-x-4 gap-y-1.5 px-4 -mt-2" dir="ltr">
        {dataWithColors.map((entry, index) => (
          ...
          >
            ...
            <span className="text-fg-muted truncate max-w-[120px]">{entry.projectName}</span>
            <span className="text-fg-secondary font-medium">{((entry.total / total) * 100).toFixed(0)}%</span>
          </div>
        ))}
      </div>
    </div>

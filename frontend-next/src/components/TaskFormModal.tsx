...
export default function TaskFormModal({ open, onClose, onSaved, projectId, departmentId }: TaskFormModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" dir="rtl">
      ...
      <div className="relative bg-card border border-line rounded-[20px] w-full max-w-xl max-h-[90vh] overflow-y-auto animate-scale-in">
        <div className="sticky top-0 bg-card border-b border-line px-6 py-4 flex items-center justify-between rounded-t-[20px] z-10">
          <h2 className="text-lg font-semibold text-fg">تسک جدید</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          <div>
            <label className="block text-sm font-medium text-fg-secondary mb-1.5">عنوان تسک *</label>
          ...
          <div>
            <label className="block text-sm font-medium text-fg-secondary mb-1.5">انجام‌دهنده *</label>
          ...
          <div className="flex gap-3 pt-2">
            <button type="submit" disabled={saving} className="flex-1 py-2.5 bg-pill hover:opacity-90 disabled:opacity-40 text-pill-fg rounded-xl font-medium transition-all duration-200">

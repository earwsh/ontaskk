...
export default function WorkloadPanel({ health, title = 'توازن بار کاری' }: WorkloadPanelProps) {
  ...
  return (
    <SectionCard title={title} icon={scaleIcon} iconColor="#6366F1" className="h-full">
      {unavailable ? (
        <div className="py-8 text-center text-xs text-fg-muted">سرویس راست در دسترس نیست</div>
      ) : (
        <div className="space-y-4">
          ...
          {health.narrative && (
            <div className="rounded-xl border border-line bg-sunken px-3.5 py-3 text-xs leading-relaxed text-fg-secondary">
              {health.narrative}
            </div>
          )}
          {health.overloaded && health.overloaded.length > 0 && (
            <div>
              <div className="mb-2 text-xs text-fg-muted">اعضای پرکار ({health.overloaded.length})</div>
              <div className="space-y-1.5">
                {health.overloaded.slice(0, 4).map((m) => (
                  <div key={m.userId} className="flex items-center justify-between rounded-lg border border-red-500/10 bg-red-500/5 px-3 py-2 text-xs">
                    <span className="text-fg-secondary">{m.name} {m.departmentName && <span className="text-fg-muted">• {m.departmentName}</span>}</span>
          ...
          {health.underloaded && health.underloaded.length > 0 && (
            <div>
              <div className="mb-2 text-xs text-fg-muted">اعضای کم‌بار ({health.underloaded.length})</div>
              <div className="space-y-1.5">

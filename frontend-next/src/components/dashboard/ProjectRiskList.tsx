...
const riskStyle = (risk?: string) => {
  if (risk === 'critical') return { badge: 'bg-bad-soft text-bad', bar: 'bg-bad', label: 'بحرانی' };
  if (risk === 'warning') return { badge: 'bg-warn-soft text-warn', bar: 'bg-warn', label: 'نیاز به توجه' };
  return { badge: 'bg-ok-soft text-ok', bar: 'bg-ok', label: 'سالم' };
};
  ...
  return (
    ...
    >
      {shown.length > 0 ? (
          ...
          {shown.map((p) => {
            ...
            return (
              <Link key={p.projectId} href={`/dashboard/projects/${p.projectId}`}
                className="block rounded-xl bg-card px-3.5 py-2.5 transition-all duration-200 hover:bg-card-hover">
                <div className="flex items-center gap-3">
                  <span className="min-w-0 flex-1 truncate text-xs text-fg">{p.projectName}</span>
                  <span className={`shrink-0 rounded-lg px-1.5 py-0.5 text-[10px] font-medium ${st.badge}`}>{st.label}</span>
                  <div className="h-1.5 w-16 shrink-0 overflow-hidden rounded-full bg-hover">
                    <div className={`h-full rounded-full transition-all duration-500 ${st.bar}`} style={{ width: `${rate}%` }} />
                  </div>
                  <span className="w-8 shrink-0 text-right text-xs text-fg-muted">{rate}٪</span>
                </div>
                {(p.total !== undefined || p.reason) && (
                  <div className="mt-1 pr-0 text-[11px] text-fg-muted">
        ...
        <div className="py-8 text-center text-xs text-fg-muted">پروژه‌ای وجود ندارد</div>
      )}

...
export default function TaskRecurrenceConfig({ value, onChange, compact = false }: TaskRecurrenceConfigProps) {
  ...
  return (
    <div className="bg-card border border-line rounded-2xl p-4 space-y-4">
      ...
      >
        <div className="flex items-center gap-3">
          <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 transition-colors ${
            value.isRecurring ? 'bg-pill text-pill-fg' : 'bg-hover text-fg-muted group-hover:text-fg'
          }`}>
          ...
          <div>
            <span className="text-sm font-semibold text-fg block group-hover:text-brand-hover transition-colors">
              تکرار خودکار تسک
            </span>
        ...
        <button
          ...
          className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
            value.isRecurring ? 'bg-primary' : 'bg-hover'
          }`}
        >
          <span
            aria-hidden="true"
      ...
      {/* Recurrence Details */}
      {value.isRecurring && (
        <div className="space-y-4 pt-2 border-t border-line animate-fade-in">
          {/* Pattern Selection Grid */}
          <div>
            <label className="block text-xs font-medium text-fg-secondary mb-2">نوع تکرار</label>
                  ...
                  <button
                    ...
                    className={`p-2.5 rounded-xl border text-right transition-all cursor-pointer flex flex-col justify-between ${
                      active
                        ...
                        : 'border-line bg-[var(--card)] text-fg-secondary hover:text-fg hover:border-line-strong'
                    }`}
                  >
                    <div className="flex items-center justify-between mb-1.5">
                      <span className="text-xs font-semibold">{p.label}</span>
                      <svg className={`w-3.5 h-3.5 ${active ? 'text-brand' : 'text-fg-muted'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                        <path strokeLinecap="round" strokeLinejoin="round" d={p.icon} />
                      </svg>
                    </div>
                    <span className="text-[10px] text-fg-muted line-clamp-2 leading-tight">{p.desc}</span>
                  </button>
                );
              })}
            </div>
          ...
          {value.recurrencePattern === 'WEEKLY' && (
            <div className="p-3 bg-card rounded-xl border border-line space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-xs text-fg-secondary font-medium">روزهای تکرار در هفته:</span>
                <span className="text-[11px] text-fg-muted">
                  ...
                  return (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all cursor-pointer ${
                        selected
                          ? 'bg-pill text-pill-fg shadow-sm'
                          : 'bg-hover text-fg-muted hover:text-fg hover:bg-hover'
                      }`}
                    >
                      {d.label}
                    </button>
          ...
          {value.recurrencePattern === 'MONTHLY_DAYS' && (
            <div className="p-3 bg-card rounded-xl border border-line space-y-2.5">
              <div className="flex items-center justify-between">
                <span className="text-xs text-fg-secondary font-medium">روزهای ماه شمسی (۱ تا ۳۱):</span>
                  ...
                  return (
                    <button
                      ...
                      className={`h-7 rounded-md text-xs font-medium transition-all cursor-pointer flex items-center justify-center ${
                        selected
                          ? 'bg-pill text-pill-fg shadow-sm font-bold'
                          : 'bg-hover text-fg-muted hover:text-fg hover:bg-hover'
                      }`}
                    >
                      {d}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 items-center">
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                {/* The picker itself offers no way back to empty, so once a
                    date was chosen the schedule could never be made open-ended
                    again. */}
                {value.recurrenceEnd && (
                  <button
                    type="button"
                    onClick={() => setRecurrenceEnd('')}
                    title="حذف تاریخ پایان — تکرار تا همیشه ادامه یابد"
                    className="shrink-0 cursor-pointer rounded-full bg-sunken px-3 py-2 text-[11px] font-medium text-fg-secondary transition-colors hover:text-fg"
                  >
                    تا همیشه
                  </button>
                )}
            ...
            <div className="text-[11px] text-fg-muted md:pt-4">

...
import { APP_TIMEZONE, clockTime } from '@/lib/date';
...
function formatPersianDate(dateStr: string) {
  if (!dateStr) return '';
  // Pinned to Tehran rather than the viewer's device, so a message does not
  // change day depending on where it is being read.
  return new Date(dateStr).toLocaleDateString('fa-IR', {
    timeZone: APP_TIMEZONE, month: 'long', day: 'numeric',
  });
}
...
export default function ChatPage() {
  ...
  return (
    <div className="h-[calc(100vh-8rem)] flex flex-col bg-card border border-line rounded-[24px] overflow-hidden shadow-2xl animate-fade-in" dir="rtl">
      <div className="flex-1 flex min-h-0">
        {/* Left Sidebar: Conversations list */}
        <div className="w-80 md:w-96 border-l border-line flex flex-col bg-[rgba(15,19,29,0.5)] shrink-0">
          {/* Sidebar Header */}
          <div className="p-4 border-b border-line flex items-center justify-between shrink-0">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-xl bg-primary/10 text-brand flex items-center justify-center">
                <svg className="w-4.5 h-4.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                </svg>
              </div>
              <h2 className="text-base font-bold text-fg">پیام‌رسان سازمانی</h2>
            </div>
            <button
              onClick={() => setShowNewChatModal(true)}
              className="px-3 py-1.5 bg-pill hover:opacity-90 text-pill-fg rounded-xl text-xs font-medium flex items-center gap-1.5 transition-all cursor-pointer shadow-sm"
            >
          ...
          <div className="p-3 border-b border-line shrink-0">
            <div className="relative">
              <input
                ...
                className="w-full pl-3 pr-9 py-2 bg-card border border-line rounded-xl text-xs text-fg placeholder-text-muted focus:outline-none focus:border-primary/50 transition-all"
              />
              <svg className="w-4 h-4 text-fg-muted absolute right-3 top-2.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          {notifPermission === 'default' && (
            <div className="p-3 bg-primary/10 border-b border-primary/20 flex items-center justify-between gap-2 shrink-0">
              <div className="flex items-center gap-2 text-brand text-[11px] min-w-0">
              ...
              <button
                ...
                className="px-2.5 py-1 bg-pill hover:opacity-90 text-pill-fg rounded-lg text-[10px] font-bold shrink-0 transition-all cursor-pointer"
              >
                فعال‌سازی
              </button>
            </div>
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-[var(--line)]">
            {loadingConv ? (
              <div className="p-8 text-center text-fg-muted text-xs">در حال دریافت گفتگوها...</div>
            ) : filteredConversations.length === 0 ? (
              <div className="p-8 text-center text-fg-muted text-xs flex flex-col items-center gap-2">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="relative shrink-0">
                      <div className="w-11 h-11 rounded-2xl bg-gradient-to-br from-primary/20 to-primary/5 border border-primary/20 flex items-center justify-center text-brand font-bold text-sm">
                        {c.type === 'GROUP' ? (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-xs font-semibold text-fg truncate block max-w-[140px]">{c.title}</span>
                        {c.lastMessage && (
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        {c.unreadCount > 0 && (
                          <span className="min-w-[18px] h-[18px] px-1 bg-pill text-pill-fg rounded-full text-[10px] font-bold flex items-center justify-center shrink-0">
                            {c.unreadCount}
                          </span>
                        )}
                      </div>
        ...
        <div className="flex-1 flex flex-col bg-[rgba(10,13,20,0.4)] min-w-0">
          {activeConv ? (
            <>
              {/* Chat Header */}
              <div className="h-16 px-6 border-b border-line flex items-center justify-between bg-card shrink-0">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-2xl bg-primary/20 border border-primary/30 flex items-center justify-center text-brand font-bold text-sm">
                  ...
                  <div>
                    <h3 className="text-sm font-bold text-fg">{activeConv.title}</h3>
                    <div className="flex items-center gap-1.5 mt-0.5">
                      {activeConv.type === 'DIRECT' && activeConv.otherUser ? (
                        ...
                        <span className="text-[11px] text-fg-muted">{activeConv.participants.length} عضو</span>
                      )}
                    </div>
                  </div>
                </div>
              ...
              <div className="flex-1 p-6 overflow-y-auto space-y-4">
                {loadingMsgs ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-full text-fg-muted text-xs gap-2">
                  ...
                  messages.map((msg) => {
                    ...
                    return (
                      ...
                      >
                        {!isMe && (
                          <div className="w-8 h-8 rounded-xl bg-card border border-line text-brand flex items-center justify-center text-xs font-bold shrink-0 mt-1">
                        ...
                        <div className="group relative">
                          <div
                            className={`rounded-2xl p-3.5 shadow-md ${
                              isMe
                                ? 'bg-pill text-pill-fg rounded-tr-sm'
                                : 'bg-[var(--card)] border border-line text-zinc-100 rounded-tl-sm'
                            }`}
                          >
                            {!isMe && activeConv.type === 'GROUP' && (
                              <p className="text-[11px] font-bold text-brand mb-1">
                            ...
                            {msg.replyTo && (
                              <div
                                className={`mb-2 p-2 rounded-lg text-xs border-r-2 ${
                                  isMe
                                    ? 'bg-black/20 border-white/60 text-fg/90'
                                    : 'bg-hover border-primary text-fg-muted'
                                }`}
                              >
                            ...
                            {/* Timestamp */}
                            <div className={`flex items-center justify-end gap-1 mt-1 text-[10px] ${isMe ? 'text-fg/75' : 'text-fg-muted'}`}>
                          ...
                          <button
                            ...
                            className={`absolute top-2 opacity-0 group-hover:opacity-100 transition-opacity p-1 bg-card border border-line-strong rounded-lg text-fg-muted hover:text-fg ${
                              isMe ? '-left-8' : '-right-8'
                            }`}
                            title="پاسخ دادن"
                          >
                ...
                {typingUsers.length > 0 && (
                  <div className="flex items-center gap-2 text-xs text-fg-muted animate-pulse">
              ...
              {/* Reply Preview Bar */}
              {replyMessage && (
                <div className="px-6 py-2 bg-[var(--card)] border-t border-line flex items-center justify-between text-xs text-fg-muted">
                  ...
                  <button onClick={() => setReplyMessage(null)} className="p-1 hover:text-fg transition-colors">
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              {/* Message Input Box */}
              <form onSubmit={handleSendMessage} className="p-4 border-t border-line bg-[rgba(15,19,29,0.7)] flex items-end gap-3 shrink-0">
                <input
                  type="file"
                  ref={fileInputRef}
                ...
                <button
                  ...
                  className="p-2.5 rounded-xl bg-hover text-fg-muted hover:text-fg hover:bg-hover transition-all cursor-pointer disabled:opacity-50 shrink-0"
                ...
                <div className="flex-1 relative">
                  <textarea
                    ...
                    className="w-full px-4 py-2.5 bg-card border border-line rounded-xl text-xs text-fg placeholder-text-muted focus:outline-none focus:border-primary/50 resize-none max-h-28 transition-all"
                  />
                </div>
                ...
                <button
                  ...
                  className="p-2.5 rounded-xl bg-pill hover:opacity-90 text-pill-fg disabled:opacity-40 disabled:cursor-not-allowed transition-all cursor-pointer shrink-0 shadow-md"
                  title="ارسال پیام"
                >
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-fg-muted text-sm gap-3">
              <div className="w-16 h-16 rounded-3xl bg-primary/10 text-brand flex items-center justify-center">
      ...
      {showNewChatModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-card border border-line rounded-[24px] w-full max-w-lg overflow-hidden shadow-2xl animate-scale-in">
            <div className="px-6 py-4 border-b border-line flex items-center justify-between">
              <h3 className="text-base font-bold text-fg">ایجاد گفتگوی جدید</h3>
              <button onClick={() => setShowNewChatModal(false)} className="text-fg-muted hover:text-fg">
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex bg-card p-1 rounded-xl border border-line">
                <button
                  ...
                  className={`flex-1 py-2 rounded-lg text-xs font-medium transition-all ${
                    chatType === 'DIRECT' ? 'bg-pill text-pill-fg shadow-sm' : 'text-fg-muted hover:text-white'
                  }`}
                >
                  چت خصوصی
                </button>
                <button
                  ...
                  className={`flex-1 py-2 rounded-lg text-xs font-medium transition-all ${
                    chatType === 'GROUP' ? 'bg-pill text-pill-fg shadow-sm' : 'text-fg-muted hover:text-white'
                  }`}
                >
                  گروه جدید
                </button>
              ...
              {chatType === 'GROUP' && (
                <div>
                  <label className="block text-xs font-medium text-fg-secondary mb-1">نام گروه *</label>
                  <input
                    type="text"
                    value={groupTitle}
                    onChange={(e) => setGroupTitle(e.target.value)}
                    placeholder="مثال: تیم فنی و توسعه"
                    className="w-full px-4 py-2.5 bg-card border border-line rounded-xl text-xs text-fg placeholder-text-muted focus:outline-none focus:border-primary/50"
                  />
                </div>
              )}
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2 bg-card border border-line rounded-xl text-xs text-fg placeholder-text-muted focus:outline-none focus:border-primary/50 mb-2"
                />
                      ...
                      <div
                        ...
                        className={`p-2.5 rounded-xl border flex items-center justify-between cursor-pointer transition-all ${
                          selected
                            ...
                            : 'bg-card border-line text-fg-secondary hover:text-fg'
                        }`}
                      >
                        <div className="flex items-center gap-2.5">
                          <div className="w-8 h-8 rounded-lg bg-primary/20 text-brand flex items-center justify-center font-bold text-xs">
                            {u.firstName.charAt(0)}
                          </div>
                          <div>
                            <span className="text-xs font-medium text-fg block">{u.firstName} {u.lastName}</span>
                            <span className="text-[10px] text-fg-muted">{u.email}</span>
                          </div>
                        </div>
                        ...
                        <div className={`w-4 h-4 rounded-md border flex items-center justify-center ${selected ? 'bg-primary border-primary' : 'border-zinc-600'}`}>
                          {selected && (
                            <svg className="w-3 h-3 text-fg" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
              ...
              <div className="flex gap-3 pt-2">
                <button
                  ...
                  className="flex-1 py-2.5 bg-pill hover:opacity-90 disabled:opacity-40 text-pill-fg rounded-xl text-xs font-medium transition-all cursor-pointer"
                >
                ...
                <button
                  ...
                  className="px-5 py-2.5 bg-card-hover text-fg-secondary hover:text-fg rounded-xl text-xs font-medium transition-all"
                >
                  انصراف
                </button>
              </div>
